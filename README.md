...
WMS 내부는 **변동량(Delta) 큐**로 관리하고, Shopify push는 **`2026-04` API 요구사항**에 맞춰 `inventorySetQuantities`로 `on_hand`를 갱신합니다. 위험도에 따라 두 트랙으로 분리됩니다.
...
- **Shopify API**: `inventorySetQuantities` (`name: "on_hand"`, `@idempotent`, `changeFromQuantity`)
- **완료**: 성공 시 해당 source 행 `isShopifyPushed = true`
...
1. `wms_inventory_changes`에서 `isShopifyPushed = false` 행 조회 (Track 2는 SKU 필터)
2. `sku + locationName` 기준으로 `deltaQty` 합산 (zero-sum 그룹 제외)
3. Shopify `locations` / `productVariants`로 location·inventoryItem ID 해석
4. Shopify 현재 `on_hand` 조회 → `changeFromQuantity` 확보
5. `quantity = changeFromQuantity + delta` 계산 후 `inventorySetQuantities` mutation 실행 (`@idempotent` 키: source row ID 해시)
6. 성공 시 source 행 `isShopifyPushed = true`로 마킹
...
>
> **동시성**: `changeFromQuantity`가 실제 Shopify 재고와 불일치하면 `CHANGE_FROM_QUANTITY_STALE`로 실패합니다. Inngest step retry 시 on_hand를 재조회합니다.
>
...
2. **Insert row** 클릭
3. 필드 입력:
   - `sku`: Shopify variant SKU (예: `TEST-SKU-001`)
...
VALUES ('TEST-SKU-001', 100, 'Shop location', true, NOW())
...
| Shopify adjust service | `src/services/shopifyInventoryAdjust.ts` | `pushPendingDeltas()` → `inventorySetQuantities` (2026-04 `on_hand`) |
...
| Phase | Action |
|-------|--------|
| Fetch | `wms_inventory_changes` where `isShopifyPushed = false` (optional SKU filter) |
| Aggregate | `sku + locationName` 기준 `deltaQty` 합산 |
| Resolve IDs | Shopify `locations` + `productVariants` → location·inventoryItem ID |
| Read on_hand | `inventoryItem.inventoryLevel.quantities(names: ["on_hand"])` → `changeFromQuantity` |
| Push | `inventorySetQuantities` (`name: "on_hand"`, `quantity = changeFromQuantity + delta`, `@idempotent`) |
| Mark | source 행 `isShopifyPushed = true` |
...
```json
{
  ...
  "idempotencyKey": "<sha256 of source row IDs>"
}
```
...
> **Early return**: `{ skipped: true, reason: "no-pending-deltas" }` 또는 `{ skipped: true, reason: "no-resolvable-deltas" }`.
...
| `KAFKA_BROKER` | `kafka:9092` | `localhost:9094` | kafkajs Producer/Consumer |
...
└── src/
    ...
    │   └── shopifyInventoryAdjust.ts  # pushPendingDeltas() → inventorySetQuantities (2026-04)
    └── workers/
        ├── bullmq/
...
```sql
INSERT INTO "WmsInventory" ("sku", "stock", "location", "isSynced", "updatedAt")
VALUES ('YOUR-REAL-SKU', 100, 'Shop location', true, NOW())
ON CONFLICT ("sku") DO UPDATE
SET "stock" = 100, "location" = 'Shop location', "updatedAt" = NOW();
```

`YOUR-REAL-SKU`와 `Shop location`을 실제 Shopify 값으로 교체.

#### 2. Track 1 테스트 — 일반 delta (bulk batch)

Kafka `wms.inventory.changes` 토픽에 일반 변동 메시지 발행 (예: 100 → 95, delta `-5`):

**PowerShell** (호스트에서 `kafka-console-producer` 사용):

```powershell
$msg = '{"sku":"YOUR-REAL-SKU","locationName":"Shop location","delta":-5}'
$msg | docker exec -i wms-mvp-local-kafka-1 kafka-console-producer `
  --bootstrap-server localhost:9092 `
  --topic wms.inventory.changes
```

> 컨테이너 이름은 `docker ps`로 확인 (`kafka` 서비스). 프로젝트 폴더명에 따라 `wms-mvp-local-kafka-1` 등으로 다를 수 있습니다.

또는 Node.js one-liner (호스트 dev):

```powershell
npx tsx -e "import { createKafkaProducer, WMS_INVENTORY_CHANGES_TOPIC } from './src/config/kafka'; const p = await createKafkaProducer(); await p.send({ topic: WMS_INVENTORY_CHANGES_TOPIC, messages: [{ value: JSON.stringify({ sku: 'YOUR-REAL-SKU', locationName: 'Shop location', delta: -5 }) }] }); await p.disconnect();"
```
...
| Step | Expected |
|------|----------|
| `push-pending-deltas` | `{ pushed: 1, attempted: 1, skipped: false }` |
| **Return** | 동일 |
...
UPDATE "WmsInventory" SET "stock" = 5, "updatedAt" = NOW() WHERE sku = 'YOUR-REAL-SKU';
```
...
| `CHANGE_FROM_QUANTITY_STALE` | push 직전 Shopify 재고 변동 | Inngest retry로 on_hand 재조회 후 재시도 |
